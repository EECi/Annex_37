...
def evaluate(schema_path,
    ...
    **kwargs
    ...
    # ========================================================================
    t_start = time.time()
    print('loading predictor (importing torch may take a while...)')
    predictor = Predictor(expt_name=kwargs['expt_name'], load=True)
    print(f'predictor load time = {time.time() - t_start}')
...
if __name__ == '__main__':
    ...
    with warnings.catch_warnings():
        ...
        results = evaluate(schema_path, tau, objective_dict, clip_level, expt_name=expt_name)

    print(results)
    with open(evaluate_file, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([expt_name, results['Price Cost'], results['Emissions Cost'], results['Grid Cost']])
