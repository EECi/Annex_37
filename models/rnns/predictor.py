...
class Predictor(BasePredictorModel):
    def __init__(self, mparam_dict=None, building_indices=(5, 11, 14, 16, 24, 29), L=168, T=48,
        ...
        expt_dir = os.path.join('models', 'rnns', 'resources', expt_name)
    ...
    def train_individual(self, building_index=None, dataset_type=None, patience=25, max_epoch=200, key=None):
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=32, shuffle=True) #, num_workers=8 conflict with os.path.exists(expt_dir), 'expt_name already taken'
        val_dataset = Data(building_index, self.L, self.T, dataset_type, 'validate', self.expt_name, key)
    ...
    def test_individual(self, building_index=None, dataset_type=None, key=None):
        ...
        dataset_params_path_y = os.path.join(
            f'{os.path.join("models", "rnns", "resources", expt_name, key, "scalar_y.pkl")}')
        ...
        for x, y in test_dataloader:
            ...
            # scaler_y
            for i in range(y_hat_np.shape[-1]):
                y_hat_transformed[:, :, i] = scaler_y.inverse_transform(y_hat_np[:, :, i])
                y_transformed[:, :, i] = scaler_y.inverse_transform(y[:, :, i])
            x_in = x[:, self.type_idx]
            x_np_transformed = scaler_y.inverse_transform(x_in.reshape(-1, 1))
            x_np_transformed = x_np_transformed.reshape(x_in.shape[0], x_in.shape[1])
            x_list.append(x_np_transformed[:, -1])
            pred_list.append(y_hat_transformed)
            error = (y_transformed[:, :, 0].squeeze() - y_hat_transformed[:, :, 0].squeeze()) ** 2
            loss_list.append(error)
            gt = np.concatenate([x_np_transformed, y_transformed.squeeze(axis=1)], axis=1)
            gt_list.append(gt)
        ...
        pred = np.concatenate(pred_list).squeeze(1)
    ...
    def compute_forecast(self, observations):
        ...
        for key in self.training_order:
            ...
            for i, value in enumerate(grouped_array):
                self.buffer[key][i].append(value)
            x = torch.tensor(self.buffer[key], dtype=torch.float32)
            x_reshape = x.reshape(1, x.shape[0], x.shape[1])
            dataset_params_path_y = os.path.join(
                f'{os.path.join("models", "rnns", "resources", self.expt_name, key, "scalar_y.pkl")}')
            scaler_y = pickle.load(open(dataset_params_path_y, 'rb'))
            output = self.models[key](x_reshape)
            predict_y = output.detach().numpy() if isinstance(output, torch.Tensor) else output[0].detach().numpy() #disgrad the uncertainty
           # print(output[1].detach().numpy().shape)
            ...
            out[dataset_type].append(predict_y_transformed.squeeze(1))#
