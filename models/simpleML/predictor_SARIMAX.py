    ...
    def compute_forecast(self, observations, t: int):
        ...
        for key in self.training_order:
            ...
            if self.use_seasonality:                
                # predictive_model = ARIMA(training_x,order=(p,d,q),seasonal_order=(P,D,Q,s),enforce_stationarity=False) #,seasonal_order=(P,D,Q,s)  ,seasonal_order=(P,D,Q,s) 
                ...
                predictive_model = SARIMAX(training_x,order=(p,d,q), seasonal_order = (P,D,Q,s),enforce_stationarity=False) #,exog=exogenous
                predictive_model_fit=predictive_model.fit(maxiter = 100, disp=False) #method_kwargs={'maxiter':300} # simple_differencing = True
            else:
