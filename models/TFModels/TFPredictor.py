"""Implementation of base pytorch-forecasting predictor model class."""
...
class TF_Predictor(BasePredictorModel):
    """Implementation of generic pytorch-forecasting prediction model
    interface for training, etc., and use with the CityLearn LinMPC controller.

    The provided class is used to perform the following activities:
    - load collections of pre-trained models (all models required
        for prediction) - via `__init__` method.
    - format CityLearn datasets - via `format_CityLearn_datasets` method.
    - create new models - via `new_model` method.
    - train models on a provided dataset - via `train_model` method.
    - perform prediction inference - via `compute_forecast` method.

    A `TF_Predictor` object contains/loads a 'model group', which is the
    collection of models required to produce all necessary predictions
    ...
    def __init__(self,
        ...
        self.model_group_path = os.path.join('models','TFModels',self.model_architecture,'resources','model_logs',model_group_name)
    ...
    def _load_model_from_dir_path(self, model_path):
        ...
        return self.pytorch_forecasting_model_class.load_from_checkpoint(best_model_path)
    ...
    def format_CityLearn_datasets(
        ...
        time_varying_unknown_reals = self._specify_time_varying_unknown_reals(model_type)
        ...
        for i,dirpath in enumerate(CityLearn_dataset_dirpaths):
            ...
            data_df = self._reformat_df(data_df, ts_name, self.time_varying_known_categoricals)
    ...
    def compute_forecast(self, observations, t: int):
        ...
        if (len(self.buffer['pricing']) < self.L) or (self.simulation_duration - t < self.T):
            ...
            base_df = self._reformat_df(base_df,'pred',self.time_varying_known_categoricals)
