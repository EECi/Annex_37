...
def main(model_group_name, UCam_ids, train_path, val_path):
    ...
    NHiTS_group = NHiTS_Predictor(model_group_name=model_group_name,load=False)
    ...
    for model_type, model_name, index in zip(model_types, model_names, building_indices):
        train_ds,val_ds = NHiTS_group.format_CityLearn_datasets([train_path,val_path], model_type=model_type, building_index=index)
        nhits_model = NHiTS_group.new_model(model_name,model_type,train_ds,pre_confirm=True)
