"""Perform nhits inference & plot dynamic timeseries of results."""
...
from models.NHiTS.NHiTS_predictor import NHiTS_Predictor

import matplotlib.pyplot as plt
import plotly.graph_objects as go
from plotly.offline import plot as ply_plot

def main(model_group_name, model_type, model_name, building_index,
    ...
    if model_type in ['load','solar']:
        assert model_name in NHiTS_group.model_names[model_type], f"{model_name} is not a valid {model_type} model in model group '{model_group_name}'."
    elif model_type in ['pricing','carbon']:
        assert model_name == NHiTS_group.model_names[model_type], f"{model_name} is not a valid {model_type} model in model group '{model_group_name}'."
    else:
        raise ValueError("`model_type` argument must be one of ('load','solar','pricing','carbon').")
    
    # grab requested nhits model
    nhits = NHiTS_group.models[model_type][model_name]

    # construct inference dataset
    batch_size = 128
    n_workers = 4 if os.cpu_count() >= 8 else int(os.cpu_count()/2)
    dataset_ds, = NHiTS_group.format_CityLearn_datasets([dataset_path], model_type=model_type, model_name=model_name, building_index=building_index)
    dataset_dl = dataset_ds.to_dataloader(train=False, batch_size=batch_size, num_workers=n_workers)

    # perform inference
    print("\nPerforming inference...")
    raw_predictions, x = nhits.predict(dataset_dl, mode="raw", return_x=True)
    print(f"Loss: {round(nhits.loss.compute().item(),4)}")

    # Plot dynamic inference timeseries chart
    print("Producing plot...")
    # ========================================================================
    max_encoder_length = NHiTS_group.L
    max_prediction_length = NHiTS_group.T
