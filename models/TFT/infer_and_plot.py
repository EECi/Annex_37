"""Perform TFT inference & plot dynamic timeseries of results."""
...
def main(model_group_name, model_type, model_name, building_index,
    ...
    # grab requested TFT model
    tft = TFT_group.models[model_type][model_name]
    ...
    raw_predictions, x = tft.predict(dataset_dl, mode="raw", return_x=True)
    print(f"Loss: {round(tft.loss.compute().item(),4)}")

    # Plot dynamic inference timeseries chart
    print("Producing plot...")
    # ========================================================================
    max_encoder_length = TFT_group.L
    max_prediction_length = TFT_group.T
...
if __name__ == '__main__':
    ...
    # specify TFT model to be used for inference
