...
def assess(predictor, schema_path, tau, building_breakdown=False, update_fre=1, **kwargs):
    ...
    with tqdm(total=env.time_steps) as pbar:
        ...
        while not done:
            ...
            buffer_past_all = predictor.buffer_record(observations)
            # update every one month update_fre month
            # Compute forecast.
            forecast_start = time.perf_counter()
            if update_fre != 0:
                if num_steps % (24 * 30 * update_fre) == 0 and num_steps > 0:
                    print(f"Num Steps:{num_steps})")
                    predictor.UpdateModel(buffer_past_all=buffer_past_all,max_epoch=50)
...
if __name__ == '__main__':
    ...
    # ==================================================================================================================
    update_fres = [12,9,6,3,1]#
    for update_fre in update_fres:  # set update frequency unit: month; 0 mean no updating
        print(f'update frequency is {update_fre} month')
        ...
        save = True
        original_folder = os.path.join("models", "dms", "resources",'analysis/linear_1')
        new_folder =os.path.join("models", "dms", "resources", f'analysis/linear_1_{update_fre}m')
        ...
        model_name = os.path.join(f'analysis/linear_1_{update_fre}m')
        ...
        results_file =os.path.join('results', 'test_online.csv')#os.path.join('results', str(update_fre)+'_month_test.csv')
        ...
        with warnings.catch_warnings():
            ...
            results = assess(predictor, schema_path, tau, building_breakdown=True, train_building_index=train_building_index, update_fre=update_fre)
