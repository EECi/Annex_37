...
class Data_update(Dataset):
    ...
    def __init__(self, building_index=5, L=48, T=24, dataset_type='load', buffer_past_all=None):
        super().__init__()
        self.dataset_type = dataset_type
        self.type2idx = {'load': 0, 'solar': 1, 'price': 2, 'carbon': 3}
        self.type_idx = self.type2idx[dataset_type]
        ...
        load_list = list(buffer_past_all['load' + '_' + str(building_index)])
        solar_list = list(buffer_past_all['solar'])
        price_list = list(buffer_past_all['price'])
        carbon_list = list(buffer_past_all['carbon'])

        load = pd.DataFrame(load_list, columns=['load'])
        solar = pd.DataFrame(solar_list, columns=['solar'])
        price = pd.DataFrame(price_list, columns=['price'])
        carbon = pd.DataFrame(carbon_list, columns=['carbon'])
        data = pd.concat([load, solar, price, carbon], axis=1)
        data.columns = ['load', 'solar', 'price', 'carbon']
        data.index.name = 'time_index'
