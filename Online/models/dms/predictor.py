...
from models.dms.utils import model_finder, Data , Data_update
...
class Predictor(BasePredictorModel):
    ...
    def __init__(self, mparam_dict=None, building_indices=None, L=168, T=48,
        ...
        if load:
            ...
            for key in self.training_order:
                ...
                buffer_dataset = Data(building_index=building_index, L=int(8760), T=self.T, dataset_type=dataset_type,
                                      version='validate')
                x, _ = val_dataset[-1]
                bu, _ = buffer_dataset[-1]
                self.buffer[key] = deque(x, maxlen=len(x))
                self.buffer_past[key] = deque(bu, maxlen=len(bu))
                if 'all' in mparam_dict.keys():
    ...
    def UpdateModel(self, max_epoch=200,buffer_past_all=None):
        """Train all models.
        patience=25 is not recallable as only train dataset will be used.
        Args:
            patience (int): Number of epochs with no improvement in validation loss before training is stopped early.
            max_epoch (int): Maximum number of epochs for which to train each model unless stopped early.
        """
        for key in self.training_order:
            self.update_individual(key=key, max_epoch=max_epoch,buffer_past_all=buffer_past_all)

    def update_individual(self, key, max_epoch=200, buffer_past_all=None):

        building_index, dataset_type = self.key2bd(key)

        # Assume that new_data is your collected new data
        Update_dataset = Data_update(building_index, self.L, self.T, dataset_type, buffer_past_all)
        train_dataloader = DataLoader(Update_dataset, batch_size=32, shuffle=True)
        # val_dataset = Data(building_index, self.L, self.T, dataset_type, new_data, 'validate')
        # val_dataloader = DataLoader(val_dataset, batch_size=32, shuffle=False)
        # early stop
        ...
        # Load old model weights before training
        model = self.models[key]
        logger = TensorBoardLogger(f'{os.path.join("models", "dms", "resources", self.expt_name)}', name=key)
        trainer = Trainer(max_epochs=max_epoch,
                          logger=logger,
        ...
        tuner = Tuner(trainer)
        lr_finder = tuner.lr_find(model, train_dataloader, min_lr=1e-6, max_lr=1e-3)
        lr = lr_finder.suggestion() if lr_finder.suggestion() is not None else 1e-3
        model.learning_rate = lr
        trainer.fit(model, train_dataloader)

        expt_dir = os.path.join('models', 'dms', 'resources', self.expt_name)
        dirs = glob.glob(os.path.join(expt_dir, key,"version_*"))
        # Sort the directories by the version number (assumes the directory name starts with "version_")
        dirs.sort(key=lambda x: int(x.split("_")[-1]))
        # The last directory in the sorted list is the one with the highest version number
        latest_dir = dirs[-1]
        dir = os.path.join(latest_dir, 'checkpoints')
        checkpoint_name = os.listdir(dir)[0]
        load_path = os.path.join(dir, checkpoint_name)
        with open(os.path.join(expt_dir, 'mparam_dict.json'), 'r') as file:
            mparam_dict = json.load(file)
            ...
            L = mparam['L']
            T = mparam['T']
        self.models[key] = self.load_model(self.models[key], load_path,**mparam)
    ...
    def load_model(self, model_class, load_path, map_location=None, **mparam):
        model = model_class.load_from_checkpoint(load_path, map_location=map_location, **mparam)
        return model

    def buffer_record(self, observations):
        """Compute forecasts given current observation.
        ...
        for key in self.training_order:
            building_index, dataset_type = self.key2bd(key)
            self.buffer_past[key].append(current_obs[dataset_type][self.building_indices.index(int(building_index))])

        return self.buffer_past