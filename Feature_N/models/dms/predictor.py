...
class Predictor(BasePredictorModel):
    def __init__(self, mparam_dict=None, building_indices=None, L=168, T=48,
                 expt_name='linear_L168_T48', results_file='results.csv', feature_number=None, load=True):
        ...
        if load:
            with open(os.path.join(expt_dir, 'mparam_dict.json'), 'r') as file:
                ...
                L = mparam['L']
                T = mparam['T']
                feature_number = mparam['feature_number']
        else:
        ...
        if load:
            ...
            for key in self.training_order:
                ...
                val_dataset = Data(building_index, self.L, self.T, dataset_type, 'validate',
                                   feature_number=self.feature_number, expt_name=self.expt_name,
                                   key=key)
                x, _ = val_dataset[-1]
                self.buffer[key] = deque(x, maxlen=len(x))

                # [deque(row, maxlen=len(x)) for row in x]#deque(x, maxlen=len(x))
                if 'all' in mparam_dict.keys():
            ...
            # Initialize a cache for selected features
        self.selected_features_cache = {}

    def selectfeature(self, building_index=5, dataset_type='load', version='train', feature_number=3,
                      dataset_dir=os.path.join('data', 'analysis')):
        # Create a unique key for caching based on method parameters
        cache_key = (building_index, dataset_type, feature_number)

        # Check if the result is already in the cache
        if cache_key in self.selected_features_cache:
            return self.selected_features_cache[cache_key]

        # ... existing code for calculating select_columns ...
        ...
        # ranking the correlation matrix
        correlations_maxtrix = correlation_matrix_specific[dataset_type].abs().sort_values(ascending=False)
        # select columns with highest correlation
        select_columns = correlations_maxtrix.index[0:feature_number]
        # Save the result in the cache
        self.selected_features_cache[cache_key] = select_columns

        return select_columns

    # Assuming that Vanilla is your model class and it inherits from a PyTorch Lightning module
    # and has the class method load_from_checkpoint
    ...
    def train(self, dataset_dir=os.path.join('data', 'analysis'), version_suffix: str = None,
        ...
        """
        # print(f'Feature number is {self.feature_number}')
        for key in self.training_order:
            self.train_individual(key=key, dataset_dir=dataset_dir, version_suffix=version_suffix, patience=patience,
                                  max_epoch=max_epoch)

    def train_individual(self, building_index=None, dataset_type=None, train_dir=None, key=None,
        ...
        # datasets
        version_suffix = '' if version_suffix is None else version_suffix
    ...
    def test_individual(self, building_index=None, dataset_type=None, key=None):
        ...
        test_dataset = Data(building_index=building_index, L=self.L, T=self.T,
                            version='test', dataset_type=dataset_type, feature_number=self.feature_number,
        ...
        dataset_params_path_x = os.path.join(
            f'{os.path.join("models", "dms", "resources", self.expt_name, key, "scalar_x_" + str(key) + ".pkl")}')
        scaler_x = pickle.load(open(dataset_params_path_x, 'rb'))

        dataset_params_path_y = os.path.join(
            f'{os.path.join("models", "dms", "resources", self.expt_name, key, "scalar_y_" + str(key) + ".pkl")}')
        ...
        for x, y in test_dataloader:
            ...
            # scaler_y_    y_hat_np[0,:, :].shape i=0 y_transformed.shape y_hat_np[1, :].shape
            for i in range(y_hat_np.shape[0]):
                y_hat_transformed[i, :] = np.squeeze(scaler_y.inverse_transform(y_hat_np[i, :].reshape(-1,1)))
                y_transformed[i, :] = np.squeeze(scaler_y.inverse_transform(y_np[i, :].reshape(-1,1))) # x.shape
            a = x.shape[0]
            b = x.shape[1]
            c = x.shape[2]
            # Reshape back to [a, b, c]
            x=scaler_x.inverse_transform(x.view(-1, c))
            x=torch.tensor(x).view(a, b, c)
            if x.ndim == 3:
                x = np.squeeze(x[:, :, 0:1]) # only the first column is the target variable
            x_list.append(x[:, -1])
            pred_list.append(y_hat_transformed)
            error = (y_transformed[:, 0] - y_hat_transformed[:, 0]) ** 2
            # x.shape x[:, :, 0:1].shape
            # scaler_x.inverse_transform(x)
            loss_list.append(error)
            gt = np.concatenate([x, y_transformed], axis=1)
            gt_list.append(gt)
    ...
    def compute_forecast(self, observations, train_building_index=None):
        ...
        for key in self.training_order:  # key='load_5'
            building_index, dataset_type = self.key2bd(key)
            current_obs = {
                ...
                'Tem': np.array(observations)[0, 3].reshape(1),
                'Hum': np.array(observations)[0, 7].reshape(1),
                'DifSolar': np.array(observations)[0, 11].reshape(1),
                'DirSolar': np.array(observations)[0, 15].reshape(1),
                'Month': np.array(observations)[0, 0].reshape(1),
                'Hour': np.array(observations)[0, 2].reshape(1),
                'Day Type': np.array(observations)[0, 1].reshape(1)
            }
            dataset_params_path_x = os.path.join(
                f'{os.path.join("models", "dms", "resources", self.expt_name, key, "scalar_x_" + str(key) + ".pkl")}')
            scaler_x = pickle.load(open(dataset_params_path_x, 'rb'))
            dataset_params_path_y = os.path.join(
                f'{os.path.join("models", "dms", "resources", self.expt_name, key, "scalar_y_" + str(key) + ".pkl")}')
            scaler_y = pickle.load(open(dataset_params_path_y, 'rb'))
            selected_features = self.selectfeature(building_index=building_index, dataset_type=dataset_type,
                                                   feature_number=self.feature_number)
            values = []
            for i in selected_features:
                values.append(current_obs[i])

            self.buffer[key].append(scaler_x.transform(np.array(values).reshape(1, -1)).reshape(-1))
            ...
            out[dataset_type].append(scaler_y.inverse_transform(self.models[key](x).detach().numpy()))
        predicted_loads = np.squeeze(np.array(out['load']))
