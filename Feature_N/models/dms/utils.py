...
class Vanilla(pl.LightningModule):
    ...
    def __init__(self, L, T, feature_number, layers=(128, 256), learning_rate=1e-3):
        super().__init__()
        self.input_dim = L* feature_number  # Update the input dimension
        self.output_dim = T
        self.h = layers
    ...
    def forward(self, x):
        # Flatten the input to match the expected shape of the input layer
...
class Data(Dataset):
    ...
    """
    def select_feature(self,building_index=5, dataset_type='load', version='train', feature_number=3,
    ...
    def __init__(self, building_index=5, L=48, T=24, dataset_type='load', version='train', feature_number=None,
                 dataset_dir=os.path.join('data', 'analysis'), expt_name=None,
                 key=None): # feature_number=2
        super().__init__()
        ...
        merged_specific_cols_df.columns = ['Month', 'Hour', 'Day Type','load', 'solar',  'price', 'carbon', 'Tem',
                                           'Hum', 'DifSolar', 'DirSolar']
        select_columns = self.select_feature(building_index=building_index, dataset_type=dataset_type,
                           feature_number=feature_number)
        data = merged_specific_cols_df[select_columns]
        data.index.name = 'time_index'
        # scale the input data
        if version=='train':
            scaler_x = scaler_pick()  # StandardScaler()  # MinMaxScaler(feature_range=(0, 1))
            scaler_y = scaler_pick()  # StandardScaler()  # MinMaxScaler(feature_range=(0, 1))
            ...
            dataset_params_dir = os.path.join("models", "dms", "resources", self.expt_name, self.key)
            os.makedirs(dataset_params_dir, exist_ok=True)
            dataset_params_path_x = os.path.join(
                f'{os.path.join("models", "dms", "resources", self.expt_name, self.key, "scalar_x_" + str(self.key) + ".pkl")}')
            with open(dataset_params_path_x, 'wb') as pkl_file:
                pickle.dump(scaler_x, pkl_file)
            dataset_params_path_y = os.path.join(
                f'{os.path.join("models", "dms", "resources", self.expt_name, self.key, "scalar_y_" + str(self.key) + ".pkl")}')
            ...
            scaled_data_x = scaler_x.transform(data)
            # print(scaled_data_x.shape) #(length, feature)
            scaled_data_y = scaler_y.transform(data[self.dataset_type].values.reshape(-1, 1))
        ...
        for i in range(len(data)):
            if i == len(data) - L - T + 1:
                ...
                self.y.append([scaled_data_y[i + L:i + L + T]
                              ])
        self.x = np.squeeze(np.array(self.x, dtype=np.float32))
        self.y = np.squeeze(np.array(self.y, dtype=np.float32))
